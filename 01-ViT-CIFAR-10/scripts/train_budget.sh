#!/usr/bin/env bash
# Each invocation starts a new run; --resume reads an immutable source checkpoint.
set -euo pipefail
...
config="$experiment_dir/configs/06-budget.json"
arguments=()
while [[ $# -gt 0 ]]; do
    case "$1" in
        --config) config=$2; shift 2 ;;
        --until|--resume) arguments+=("$1" "$2"); shift 2 ;;
...
# Refuse to launch when basic host/SSD headroom is no longer available.
"$python_bin" - <<'PY'
import os, shutil
